...
class MatchRepository {
  ...
  private globalFilters(filters: SelectFilters) {
    ...
    if (typeof filters.queue !== 'undefined') query += ` AND matches.gamemode IN (${filters.queue.join(',')}) `
  ...
  public async getNextMatchIds(filters: SelectFilters) {
    ...
    if (typeof filters.queue !== 'undefined') {
      matchListQuery
      .join('matches', 'summoner_matchlist.match_id', 'matches.id')
      .whereIn('matches.gamemode', filters.queue)
    }
