...
export default class MatchesController {
  ...
  public async index({ request, response }: HttpContextContract) {
    const { puuid, region, lastMatchId, season, queue } = await request.validate(MatchesIndexValidator)
    
    const queueA: any = typeof queue !== 'undefined' ? queue.split(',').map(x => Number(x)) : queue

    const matchIds = await MatchRepository.getNextMatchIds({ lastMatchId, puuid, season, queue: queueA })
    ...
    const stats = await StatsService.getSummonerStats(puuid, season, queueA)
